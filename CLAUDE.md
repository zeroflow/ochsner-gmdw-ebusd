...
- **Schaltzyklen (~70166) + Betriebsstunden (~17137): NOT capturable — confirmed dead-end
  2026-06-30, don't retry. Full evidence in `findings.md`.** Decisive proof: on a cellar-display
  **cold boot**, the display (master `01`) freshly read ~25 controller datapoints — incl. the
  energy block `7d87..7d8c` — and **not one** is these counters; no `NN=0x0c` (4-byte) telegram
  ever crosses our wire. It's NOT the wiring (we see the display's reads fine) and NOT the
  invisible sub-bus (the energy counters ARE controller datapoints and get read). **The display
  computes/stores cycles+hours itself (own NVRAM) from the compressor status it polls — there is
  no eBUS telegram for them.** Energy kWh/MWh ARE reachable and mapped; cycles/hours are not.
